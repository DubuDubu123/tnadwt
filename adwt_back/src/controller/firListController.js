...
exports.getFirList = (req, res) => {
  ...
  const query = `SELECT * FROM fir ORDER BY created_at DESC`; // Fetch all FIRs sorted by created_at
  ...
  exports.deleteFir = (req, res) => {
    ...
    const query = `DELETE FROM fir WHERE fir_id = ?`;
  ...
  exports.updateFirStatus = (req, res) => {
    ...
    const query = `UPDATE fir SET status = ?, updated_at = NOW() WHERE fir_id = ?`;
