...
// // Update an existing FIR (includes status update)
// exports.updateFir = (req, res) => {
//   const firId = req.params.id;
//   const firData = req.body;

//   const query = `
//     UPDATE fir 
//     SET police_city = ?, police_zone = ?, police_range = ?, revenue_district = ?, police_station = ?, status = ?
//     WHERE fir_id = ?`;

//   const { policeCity, policeZone, policeRange, revenueDistrict, alphabetSelection, stationNumber, stationName, status } = firData;
//   const policeStation = `${alphabetSelection}-${stationNumber}-${stationName}`;
//   const values = [policeCity, policeZone, policeRange, revenueDistrict, policeStation, status, firId];

//   db.query(query, values, (err) => {
//     if (err) {
//       return res.status(500).json({ message: 'Failed to update FIR', error: err });
//     }
//     res.status(200).json({ message: 'FIR updated successfully' });
//   });
// };
